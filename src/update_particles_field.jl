...
function update_particles_field!(particles::Particles{T}, alg::FMMGPU; lambda) where {T}
    ...
    # d_pr_efields = CuArray{SVector{3,T}}(undef,N)
    # d_pr_bfields = CuArray{SVector{3,T}}(undef,N)
    ...
    nt = 512
    nb = ceil(Int, N/nt)
    @cuda blocks=nb threads=nt gpu_update_amplitude!(d_pr_efields, d_pr_bfields, N, amp)
