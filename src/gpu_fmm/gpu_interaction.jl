...
function gpu_M2L!(mp_gammas::CuDeviceArray{T,4,1}, mp_momenta::CuDeviceArray{SVector{3,T},4,1}, mp_efields::CuDeviceArray{SVector{3,T},4,1}, mp_bfields::CuDeviceArray{SVector{3,T},4,1}, degree::Val{D},
    ...
    m2l_lists::CuDeviceVector{Tuple{I,I},1}, m2l_lists_ptrs::CuDeviceVector{I,1}, p_avg::SVector{3,T}) where {I,T,D}
    bid = blockIdx().x
    t_i, t_j, t_k = threadIdx()
    for pairindex in m2l_lists_ptrs[bid]:(m2l_lists_ptrs[bid+1]-1)
        tindex, sindex = m2l_lists[pairindex]
...
function gpu_P2P!(pr_positions::CuDeviceVector{SVector{3,T},1}, pr_momenta::CuDeviceVector{SVector{3,T},1}, pr_efields::CuDeviceVector{SVector{3,T},1}, pr_bfields::CuDeviceVector{SVector{3,T},1}, ct_parindices::CuDeviceVector{I,1},
    cl_parlohis::CuDeviceVector{Tuple{I,I},1}, VN0::Val{N0}, p2p_lists::CuDeviceVector{Tuple{I,I},1}, p2p_lists_ptrs::CuDeviceVector{I,1}) where {I,T,N0}
    bid = blockIdx().x
    tid = threadIdx().x
    for pairindex in p2p_lists_ptrs[bid]:(p2p_lists_ptrs[bid+1]-1)
        tindex, sindex = p2p_lists[pairindex]
